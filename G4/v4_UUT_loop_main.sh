...
#path_go(){
#cd /root/system_stress
#}
#path_go
#sleep 2
#echo
#ScriptPath=/root/system_stress

ipmitool raw 0x30 0x02 2> /dev/null 1> /dev/null
source $ScriptPath/Client_configuration

echo "no" > /root/script.status
...
function CHECKRESULT(){ #check all test results and show it on the screen.
	echo
		cat "$pwd"BMC/All_SEL_analysis.log
	echo
	...
	if [ -s "$pwd"Fail_items_detail.log ];then
		...
		echo "======= $boot ======" ;echo " "; cat "$pwd"Fail_items_simple.log ;cat $ScriptPath/FAIL
	else
		echo "====== $boot ======" ;echo " " ;cat $ScriptPath/PASS
...
function OFFMETHOD(){
	...
	if [ "$FinishTest" == "Yes" ];then
		...
		echo "done" > /root/script.status		
		
		if [ "$Testmethod" == "AC" ];then
			echo "ifconfig $os_NIC_1 $os_ip_1"
			finish_counter=0
			while true
			do
				ifconfig $os_NIC_1 $os_ip_1
				sleep 5
				ping $os_ip_1 -c 5 2> /dev/null 1>ping_tmp.log
				ping_ip=$(cat ping_tmp.log | grep -i ttl -c)
				if [ "$ping_ip" == "5" ]; then
					cat ping_tmp.log 
					if [ "$finish_counter" == 2 ]; then
						break
					fi
	...
	if [ "$Testmethod" == "Reboot" ];then
		...
		echo
			cat $ScriptPath/reboot.method | grep 1 > /dev/null
				if [ "$?" == 0 ]; then	
					echo "System reboot after 60 seconds"
					counter_delay 60
					init 6
				else
					echo "BMC force reboot system after 60 seconds"
					counter_delay 60
	...
	if [ "$Testmethod" == "DC" ];then
		...
		if [ "$DC" == 1 ] && [ "$cycle_BMC" == 1 ]; then
			...
			echo "BMC DC cycle after 60 seconds"
				counter_delay 60
		...
		if [ "$DC" == 1 ] && [ "$cycle_RTC" == 1 ]; then
			...
			#	counter_delay 60
				...
				if [ "$?" == 0 ]; then	
					echo
					echo "RTC DC off after 60 seconds"
					counter_delay 60
					init 0
				else
					echo
					echo "BMC DC off after 60 seconds"
					counter_delay 60
	...
	if [ "$Testmethod" == "AC" ];then
		...
		do
			...
			ifconfig $os_NIC_1 $os_ip_1
			sleep 3
			ping $os_ip_1 -c 10 2> /dev/null 1>ping_tmp.log
			ping_ip=$(cat ping_tmp.log | grep -i ttl -c)
			if [ "$ping_ip" -gt "8" ]; then
				cat ping_tmp.log  | head -10
				#break
			else
				echo  -ne "."
				sleep 1
...
function Record_BMC(){
	## BMC FRU
	ipmitool fru print 2> "$pwd"BMC/fru/fru_$boot.log 1> "$pwd"BMC/fru/fru_$boot.log 
        sleep 1	
	ipmitool mc info 2> "$pwd"BMC/mc/mc_info_$boot.log 1> "$pwd"BMC/mc/mc_info_$boot.log
	sleep 1
	if [ "$boot" != "1" ];then
	
		echo "compare fru: $boot" >> "$pwd"BMC/fru/fru_tmp.log
		diff -c "$pwd"BMC/fru/fru_Golden_Sample.log "$pwd"BMC/fru/fru_$boot.log >> "$pwd"BMC/fru_fail.log
		echo "compare mc: $boot" >> "$pwd"BMC/mc/mc_tmp.log
		diff -c "$pwd"BMC/mc/mc_Golden_Sample.log "$pwd"BMC/mc/mc_info_$boot.log >> "$pwd"BMC/mc_fail.log
	fi

    ipmitool sel elist 2> "$pwd"BMC/event/BMC_event_$boot.log 1> "$pwd"BMC/event/BMC_event_$boot.log

	i_sdr=1
	while true
	do
		ipmitool sdr list all 2> "$pwd"BMC/sdr/sdr_list_all_$boot.log 1> "$pwd"BMC/sdr/sdr_list_all_$boot.log
		ipmitool sdr elist 2> "$pwd"BMC/sdr/sdr_$boot.log 1> "$pwd"BMC/sdr/sdr_$boot.log
		ipmitool sensor list 2> "$pwd"BMC/sdr/sensor_$boot.log 1> "$pwd"BMC/sdr/sensor_$boot.log
		cat "$pwd"BMC/sdr/sdr_$boot.log	| awk -F "|" '{print $1 $3}' > "$pwd"BMC/sdr/sdr_tmp_$boot.log
		diff -c "$pwd"BMC/sdr/sdr_Golden_tmp.log "$pwd"BMC/sdr/sdr_tmp_$boot.log > "$pwd"BMC/sdr/sdr_compare_$boot.log
	        ! test -s "$pwd"BMC/sdr/sdr_compare_$boot.log
		if [ "$?" == 0 ]; then
			break
		else
			sleep 5
			...
			#echo " =============== No. "$boot" Boot ================= " > "$pwd"BMC/BMC_sdr_tmp_$boot.log
				cat "$pwd"BMC/sdr/sdr_compare_$boot.log >> "$pwd"BMC/sdr/tmp/BMC_sdr_tmp_$boot.log
				cat "$pwd"BMC/sdr/tmp/BMC_sdr_tmp_$boot.log  >> "$pwd"BMC/BMC_sdr_fail.log
				 
				break
			fi
		fi
	done
#rm -f "$pwd"BMC/BMC_sdr_tmp_$i.log
#rm -f "$pwd"BMC/sdr/sdr_compare_$i.log	
...
repeat_SDR(){
	...
	#_________________________________________________________________________________________________
	## record BMC event 
   #echo "ipmitool sel elist > "$pwd"BMC/event/BMC_event_$boot.log"
...
#	cat "$pwd"BMC/event/BMC_event_$boot.log | awk -F " " '{print $1}' > "$pwd"BMC/event/sel_number.log
	...
	#while read line
	#do
	#ipmitool sel save  "$pwd"BMC/event/Deatil_BMC_event_$boot.log
...
Counter |                                 events 
-------------------------------------------------------------------------------------------------"
grep -v "OEM record" $1BMC/event/BMC_event_*.log | cut -d "|" -f 4- | sort | uniq -c
grep "OEM record" $1BMC/event/BMC_event_*.log | cut -d "|" -f 2- | sort | uniq -c
}
...
$ScriptPath/Get_vpd_info.sh > "$pwd"VPD/VPD_info_$boot.logdcboot.method
	#if [ "$boot" != "1" ];then
...
Record_mptsas
gnome-terminal --geometry=80x20+200+200 -- bash -c "echo Script statdcboot.methodus in Record_USB;sleep 10" 
Record_USB
...
debug_test(){
for test_item in Record_PCI Record_DMI Record_disk Record_BMC Analysis_BMC_Dmesg 
do
...
if [ "$boot" -ge "$TestCycles" ]; then
	...
	if [ "$Testmethod" == "Reboot" ];then
		...
		tar zcvf $HOME/$Backup_name"_"$Logfoldername.tar.gz $HOME/$Logfoldername/
