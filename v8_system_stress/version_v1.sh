	...
	if [ "$Testmethod" == "DC" ];then
		...
		if [ "$DC" == 1 ] && [ "$cycle_RTC" == 1 ]; then
			...
			if [ "$purestorage" == 0 ]; then
                ...
                ipmitool raw 0x30 0x17 0x02 0x00
			else
...
while true
do
ipmitool raw 0x30 0x01 | grep -i 00
if [ "$?" == 0 ]; then
    break
else
    ipmitool raw 0x30 0x02
...
echo "========== $boot ===========" >> /root/psu.log
ipmitool sensor | grep -i psu >> /root/psu.log
Shutdown_time
