#!/bin/bash
path_go(){
cd /root/Desktop/v3_system_stress
...
sleep 2
echo
ScriptPath=/root/Desktop/v3_system_stress
#!/bin/bash
#path_go(){
...
function Record_BMC(){
	...
	do
		...
		diff -c "$pwd"BMC/sdr/sdr_Golden_tmp.log "$pwd"BMC/sdr/sdr_tmp.log > "$pwd"BMC/sdr/sdr_compare.log
...
rm -f "$pwd"BMC/sdr/BMC_sdr_tmp.log
...
function Analysis_BMC_Dmesg(){
	...
	grep -i 'error\|timeout\|err\|fail\|going\|upper\|lower\|Critical\|ECC' "$pwd"BMC/event/BMC_event_$boot.log >> "$pwd"BMC/SEL_abnormal_fail.logens80f0np0
	fi
